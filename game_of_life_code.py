...
class GameOfLife:
    ...
    def __init__(self):
        ...
        self.current_generation = [[self.number_dead
                                    for col in range(self.y_cells)]
                                   for row in range(self.x_cells)]
        self.next_generation = [[self.number_dead
                                 for col in range(self.y_cells)]
                                for row in range(self.x_cells)]
        ...
        self.init_gen(self.current_generation, self.number_dead)
    ...
    def create_window(self):
        ...
        tk.Label(self.instruction_window, text='Welcome to this version of the Game of Life.'
                                               ...
                                               '\n\n The game follows the standard B3/S23 rules.',
    ...
    def reset_button(self):
        ...
        self.next_iteration = False
        self.init_gen(self.next_generation, self.number_dead)

    def quit_button(self):
        """Button to quit the game."""
        self.game_over = True

    def init_gen(self, generation, cell_state):
        """Initializing the cells."""
        for row in range(self.y_cells):
            for col in range(self.x_cells):
                generation[col][row] = cell_state
    ...
    def draw_cell(self, x, y, cell_state):
    ...
    def update_gen(self):
        ...
        for row in range(self.y_cells):
            for col in range(self.x_cells):
                cell_state = self.next_generation[col][row]
                self.draw_cell(col, row, cell_state)
    ...
    def activate_living_cell(self, x, y):
        ...
        self.next_generation[x][y] = self.number_alive

    def deactivate_living_cell(self, x, y):
        """Deactivates a cell in the next generation."""
        self.next_generation[x][y] = self.number_dead
    ...
    def check_cells(self, x, y):
        ...
        if (x >= self.x_cells) or (y >= self.y_cells):
            return 0
        if self.current_generation[x][y] == self.number_alive:
            return 1
        else:
            return 0
    ...
    def create_next_gen(self):
        ...
        for row in range(self.y_cells):
            for col in range(self.x_cells):
                ...
                if current_cell == self.number_alive:
                    if num_neighbors < 2:  # Rule number 1.
                        self.next_generation[col][row] = self.number_dead
                    elif num_neighbors > 3:  # Rule number 3.
                        self.next_generation[col][row] = self.number_dead
                    else:  # Rule number 2.
                        self.next_generation[col][row] = self.number_alive
                elif current_cell == self.number_dead:
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            x_pos = int(posn[0] / self.cell_size)
            y_pos = int(posn[1] / self.cell_size)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    if self.current_generation[x_pos][y_pos] == self.number_dead:
                        self.activate_living_cell(x_pos, y_pos)
                    else:
                        self.deactivate_living_cell(x_pos, y_pos)
